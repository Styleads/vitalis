...
#### ResourceType (enum)
`class ResourceType(str, Enum)` with members `BED, ICU_BED, OR, DOCTOR, NURSE, AMBULANCE` (a `str` enum so it works as an enum key in Python and as a plain string key in JSON; AMBULANCE exists but no default bundle uses it in v1).
...
`wait_s(now) = now - arrival_time - (service_time - remaining_service)`, i.e. total time spent waiting so far. It grows while a patient is WAITING, **including after an interruption** (an interrupted patient keeps aging in the queue), and freezes while IN_TREATMENT. Interruptions never reset `arrival_time`. `first_wait_s = start_time - arrival_time` (first start only) is what `stats_raw` reports as the patient's wait.

#### PatientView and StateView (what strategies see; frozen, read-only)
- `PatientView`: `id, urgency, arrival_time, wait_s` (as defined above, at `now`), `required, predicted_service_time, remaining_service, arrival_mode, department, interruptions`.
- `StateView` (literal field names, locked): `clock: int`, `free: Mapping[ResourceType, int]`, `total: Mapping[ResourceType, int]` (active units = not FAILED/OFF), `waiting: tuple[PatientView, ...]`, `in_treatment: int`, `flags: Mapping[str, float]` (e.g. `arrival_multiplier`). Example: `state.free[ResourceType.ICU_BED]`.
...
#**Strategy naming:** `snapshot().strategy_name` is `strategy.__name__`. Person B's registry keys must equal the function names: `urgency_only`, `urgency_wait`, `urgency_wait_util`. `engine/` never imports `strategies/`; C imports `strategies.STRATEGIES` and passes the function into `Engine`, `set_strategy`, and `run_headless`. B imports only the frozen view types from `engine.types`.
...
#### stats_raw()
```
{ clock,                                                    # final simulated time, needed to compute censored waits
  patients: [ {id, urgency, arrival, start, end, interruptions, status} ],   # start/end are None until they happen
...
- tests/engine/test_policies.py: BACKFILL vs BLOCK with an ICU-blocked head
  ...
  cancelled TREATMENT_DONE (lazy deletion); an interrupted patient's `PatientView.wait_s` keeps growing while requeued and equals `now - arrival - treated seconds`.
